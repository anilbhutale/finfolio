...
class InvoiceRetrieveView(RetrieveAPIView):
    ...
    def get(self, request, invoice_id=None, format=None):
        category_id = request.query_params.get("category", None)  # Get category ID from query params
        if invoice_id:
            invoices = Invoice.objects.filter(Q(id__contains=invoice_id))
        elif category_id:
            invoices = Invoice.objects.filter(category=category_id, user=request.user)  # Filter by category and user
        else:
            invoices = Invoice.objects.filter(user=request.user)
        serializer = InvoiceListSerializer(invoices, many=True)
        return Response(serializer.data)
